...
// C++ will only "discriminate" the type of an object being messaged, not the type of an object being passed.  
// So in order for the type of 2 objects to be discriminated, each object must be the receiver of a virtual function call.
// Here, when process1() is called on the first object, its type becomes "known" at runtime, but the type of the second is still UNknown.
